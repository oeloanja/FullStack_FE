...
export default function BorrowerSignup() {
  ...
  const handleSendVerification = async () => {
    setIsLoading(true)
    try {
      const response = await api.post('/api/v1/user-service/users/borrow/email/verification', {
  ...
  const handleVerifyCode = async () => {
    setIsLoading(true)
    try {
      const response = await api.post('/api/v1/user-service/users/borrow/email/verify', {
        email: formData.email,
